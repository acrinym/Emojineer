...
case OpCode::AssertArray:if(!std::holds_alternative<ArrayPtr>(peek(line)))runtime_error(line,"📚 variable requires an array");break;
...
case OpCode::MakeArray:{auto count=static_cast<std::size_t>(ins.operand);if(stack_.size()<count)runtime_error(line,"not enough values for array literal");auto a=std::make_shared<ArrayValue>();a->elements.resize(count);for(std::size_t n=count;n>0;--n)a->elements[n-1]=pop(line);stack_.emplace_back(std::move(a));break;}
case OpCode::Index:{Value iv=pop(line),cv=pop(line);auto*ap=std::get_if<ArrayPtr>(&cv);if(!ap||!*ap)runtime_error(line,"🔎 requires an array");double raw;if(auto*d=std::get_if<double>(&iv))raw=*d;else if(auto*i=std::get_if<std::int64_t>(&iv))raw=static_cast<double>(*i);else runtime_error(line,"🔎 index must be a whole number");if(!std::isfinite(raw)||std::floor(raw)!=raw||raw<0||raw>=static_cast<double>((*ap)->elements.size()))runtime_error(line,"🔎 array index out of range");stack_.push_back((*ap)->elements[static_cast<std::size_t>(raw)]);break;}
case OpCode::Length:{Value v=pop(line);if(auto*ap=std::get_if<ArrayPtr>(&v)){if(!*ap)runtime_error(line,"📏 received a null array");stack_.emplace_back(static_cast<double>((*ap)->elements.size()));}else if(auto*text=std::get_if<std::string>(&v))stack_.emplace_back(static_cast<double>(segment_graphemes(*text).size()));else runtime_error(line,"📏 requires an array or text value");break;}
case OpCode::Append:{Value value=pop(line),cv=pop(line);auto*ap=std::get_if<ArrayPtr>(&cv);if(!ap||!*ap)runtime_error(line,"📎 requires an array");auto next=std::make_shared<ArrayValue>(**ap);next->elements.push_back(std::move(value));stack_.emplace_back(std::move(next));break;}
case OpCode::SetIndex:{Value value=pop(line),iv=pop(line),cv=pop(line);auto*ap=std::get_if<ArrayPtr>(&cv);if(!ap||!*ap)runtime_error(line,"🧷 requires an array");double raw;if(auto*d=std::get_if<double>(&iv))raw=*d;else if(auto*i=std::get_if<std::int64_t>(&iv))raw=static_cast<double>(*i);else runtime_error(line,"🧷 index must be a whole number");if(!std::isfinite(raw)||std::floor(raw)!=raw||raw<0||raw>=static_cast<double>((*ap)->elements.size()))runtime_error(line,"🧷 array index out of range");auto next=std::make_shared<ArrayValue>(**ap);next->elements[static_cast<std::size_t>(raw)]=std::move(value);stack_.emplace_back(std::move(next));break;}
