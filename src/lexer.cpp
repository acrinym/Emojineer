...
std::vector<Token> Lexer::tokenize() const {
    ...
    for(size_t i=0;i<gs.size();){const auto&g=gs[i];
        ...
        std::ostringstream msg;msg<<"unexpected grapheme '"<<g.display<<"' ("<<codepoints_hex(g.display)<<")";throw lsp::SourceLocationException(msg.str(),{},line,col,g.display);
    }
