...
Chunk Compiler::compile(const ast::Program& program){
    ...
    for(const auto&s:program.statements){
        if(auto*f=dynamic_cast<const ast::FunctionDecl*>(s.get())){
            if(function_indices_.find(f->name) != function_indices_.end())throw std::runtime_error("duplicate function '"+f->name+"'");
    ...
    emit(OpCode::Halt,0,static_cast<std::size_t>(1));  // Halt at line 1, no source info
...
void Compiler::emit(OpCode op,std::int32_t operand,std::size_t line,std::size_t column){
    ...
    // Default to line/column when no source range provided
...
// Overload: emit with source range from AST
void Compiler::emit(OpCode op, std::int32_t operand, const ast::SourceRange* source) {
    ...
    // Use per-node module identity if set, otherwise fall back to global source_path
    std::string module_path = source && !source->module_identity.empty() 
