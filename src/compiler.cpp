...
Chunk Compiler::compile(const ast::Program& program){
    ...
    for(const auto&s:program.statements){
        if(auto*f=dynamic_cast<const ast::FunctionDecl*>(s.get())){
            ...
            if(auto intrinsic=intrinsic_from_identifier(f->name))
                throw SourceLocationException("reserved intrinsic emoji '"+intrinsic_glyph(*intrinsic)+"' cannot be defined as a function",{},f->line,1,f->name);
    ...
    for(const auto&s:program.statements){
        if(auto*decl=dynamic_cast<const ast::InteropImportDecl*>(s.get())){
            ...
            if(intrinsic_from_identifier(decl->name))throw SourceLocationException("interop import cannot shadow a reserved intrinsic",{},decl->line,1,decl->name);
