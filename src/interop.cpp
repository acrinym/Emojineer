...
void encode_value(Writer&w,const Value&v,std::size_t depth,std::size_t&elements){
    ...
    if(auto p=std::get_if<std::int64_t>(&v)){w.u8(static_cast<std::uint8_t>(ValueTag::Integer));w.u64(static_cast<std::uint64_t>(*p));return;}
