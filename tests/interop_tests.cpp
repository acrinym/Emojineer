...
void test_abi_roundtrip_and_bounds(){
 emojineer::InteropSignature sig{{emojineer::InteropType::Array},emojineer::InteropType::Array};auto inner=std::make_shared<emojineer::ArrayValue>();inner->elements={std::int64_t{7},2.5,true,std::string("hi")};auto outer=std::make_shared<emojineer::ArrayValue>();outer->elements={inner,std::string("tail")};std::vector<emojineer::Value> args{outer};auto request=emojineer::encode_interop_request(sig,args);auto decoded=emojineer::decode_interop_request(sig,request);require(emojineer::values_equal(decoded[0],args[0]),"request codec preserves nested values and numeric subtypes");auto response=emojineer::encode_interop_success(emojineer::InteropType::Array,decoded[0]);require(emojineer::values_equal(emojineer::decode_interop_response(emojineer::InteropType::Array,response),args[0]),"response codec is equivalent");
